...
\item \textbf{Refinement (Phase 5).} \textbf{Done} for static refinement,
      ...
      gravity path on refined meshes (wired, untested), and the linA finding of the
      validation document (a $10^{-11}$ linear mode grows at half its rate through radial
      level boundaries; the minmod prolongation of the gas and of $u^*$ is the first
      suspect).
...
\code{time0}, \code{remap}, \code{fold}) and \code{dust\_nsh}'s \code{lattice\_finest} (\S\ref{sec:phase5}),
