...
exchange was a percent-level error.
...
4b & (i) \textbf{done} & (i) shear-periodic \emph{additive} deposit remap
     (\S\ref{sec:phase4b1}); (ii) ideal-gas energy update under drag back-reaction;
     (iii) particle timestep on the shear-subtracted velocity; adopt the PC2 coupling
     (\S\ref{sec:merge2}) as the production path \\
...
(next) & 4b (i) & \fn{MeshBoundaryValuesDep::FoldShearDeposit}, the $x_1$-buffer skip,
