...
\title{Dust Particles and Gas$+$Dust Self-Gravity\\
       ...
       gravity on statically refined meshes}
\author{\code{athenak-multigrid} tree, branches \code{dust-multigrid} and \code{dust-smr}\\
        ...
        \code{386d5968}; Phase 5 and gravity on refined meshes on \code{dust-smr}:
        \code{04dbd3dd}, \code{af45c756}, \code{c3d7b308}}
\date{September 3--10, 2026}
...
orbital advection, and at most one level of refinement for the dust (the reason is in
...
\item \textbf{Refinement (Phase 5).} \textbf{Done} for static refinement,
      ...
      shear machinery at the face level. Still open: adaptive refinement (a particle
      redistribution hook next to \fn{ShearingBox::ReinitAfterMeshUpdate}), more than one
      level of dust refinement, the coupled drag solvers under refinement,
      and particle splitting on entry
      into a finer level: a coarse-sampled population keeps its coarse sampling there
      (the linA diagnosis of the validation document, \S10.5: a lattice at twice the
      fine spacing is aliased by the fine kernel into an $O(1)$ comb). Until then the
      particle count is set by the finest level of the dust layer; the test generators
      \code{dust\_nsh} and \code{streaming\_linear} have \code{lattice\_finest}.
      \code{<mesh\_refinement>/prolong\_limiter = none} (unlimited central slopes in
      \fn{ProlongCC}) is available as a diagnostic.
...
\multicolumn{3}{@{}p{15.4cm}@{}}{\small Static refinement (\code{<mesh\_refinement>/refinement = static}) is
  supported with one level, MeshBlocks of $\ge 2\,$\code{nghost} cells per direction and
  \code{drag\_solver = local} (\S\ref{sec:phase5}); \code{adaptive} is fatal.} \\
