...
\title{Dust Particles and Gas$+$Dust Self-Gravity\\
       ...
       \large Phase 4a: the dust-module merge}
...
4b & next & shear-periodic \emph{additive} deposit remap; ideal-gas energy update under
     drag back-reaction; particle timestep on the shear-subtracted velocity; adopt the
     PC2 coupling (\S\ref{sec:merge2}) as the production path \\
...
advection ($\sim 5\times$ at SC14 size). Phase 4b relaxes it (\S\ref{sec:next}).
...
\item \textbf{Shear-periodic additive deposit remap (4b).} The $u^\ast$ copy exchange
      is shear-remapped, the two additive deposits ($Q,\mathbf P$ and the back-reaction
      momentum) are not; the module warns and is exact only for axisymmetric states.
      The Phase-4c gravity deposit will be a third additive field. Design: gather the
      $x_1$ ghost slabs of the boundary blocks, shift in $y$ by $\pm y_{\rm sh}$ with
      the conservative remap kernels of \file{remap\_fluxes.hpp} (the Phase-1
      machinery; PPMX default), and \emph{add} into the receiving active strips ---
      i.e.\ a \code{ShearingBoxCC}-style pack/remap with additive unpack. Mass is
      conserved exactly by construction; the remap's smoothing touches only the
      ghost-sourced part of the boundary columns. The alternative (image particles
      deposited directly in the neighbor's frame) is exact but needs a second particle
      exchange; keep it in reserve.
\item \textbf{Ideal-gas back-reaction (4b).} \fn{ApplyPMBR} updates momentum only, so
      back-reaction requires an isothermal EOS, while gravito-turbulence needs cooling.
      Add $\Delta E = \Delta(|\mathbf M|^2/2\rho)$ (internal energy fixed under the drag
      kick), with frictional heating as a flagged option.
\item \textbf{Particle timestep under shear (4b).} Limit on $v_y$, not $v_y - \qom x$,
      with a guard that the per-stage azimuthal jump stays below a MeshBlock width
      (migration is position-based and finds the destination by $(y,z)$ lookup, so
      multi-cell jumps are legal). Otherwise every dust run in a wide box pays the
      non-FARGO factor.
...
boundaries, or shear-periodic $x_1$ in 3D; \code{<hydro>/eos = isothermal} whenever
\code{back\_reaction = true}. Shearing-box runs read \code{<shearing\_box>/qshear},
...
\code{dt\_cfl} & 0.5 & particle transport CFL factor \\
...
seeds, eigenmode amplitudes) are specific to the four built-in generators
