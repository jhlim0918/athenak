...
\begin{enumerate}
  ...
  \item \textbf{(done; Sec.~\ref{sec:phase0})} Phase 0: shearing box + mesh refinement
        infrastructure --- interior-$x_1$ refinement policy, a non-FARGO mode, and
        orbital advection (FARGO) on refined meshes under an annular refinement policy.
  \item Phase 1--2: shear-periodic boundary conditions in the multigrid driver,
        validated against the FFT oracle; then multigrid gravity on refined
        shearing-box meshes.
...
%=======================================================================
\section{Phase 0: shearing box with mesh refinement, and FARGO on refined meshes}
\label{sec:phase0}

AthenaK previously refused shearing box + refinement outright
(\code{src/mesh/mesh.cpp}, a \code{FIXME} fatal). Earlier experiments on the
\code{athenak} branch \code{test/shwave-smr} showed SMR works if refined blocks avoid
the shear-periodic $x_1$ boundaries --- but only with orbital advection (FARGO)
disabled, and without FARGO the timestep penalty is a factor
$\sim (c_s + q\Omega L_x/2)/c_s$ ($\approx$10--30 for gravito-turbulence boxes).
Phase 0 folded that branch into \code{athenak-multigrid} (0a) and then made FARGO
itself work on refined meshes (0b).

\subsection{What was implemented}
\textbf{0a (ported from \code{test/shwave-smr}, commit \code{e7ef37f2}):}
a \code{<shearing\_box> orbital\_advection = true|false} toggle; in non-FARGO mode the
solver integrates the full shear flow (full Coriolis $+\,2q\Omega^2x$ tidal source
terms with tidal work, azimuthal momentum/energy offsets of $\pm\rho\,q\Omega L_x$ in
the shear-periodic BC wrap, background shear included in pgen initial conditions).
The blanket prohibition became an enforced policy
(\code{Mesh::CheckShearingBoxRefinement}, fatal at tree build, restart, and after every
AMR update): \emph{refined MeshBlocks must not touch the shear-periodic $x_1$
boundaries}, plus an AMR-side refinement veto for boundary blocks.

\textbf{0b (per-level FARGO under the annular policy):} the orbital-advection shift
kernel is already per-block (each MeshBlock shifts its own $x$-columns by
$q\Omega\,x\,dt$ in its own $\Delta y$), so the only refinement-incompatible piece was
the communication pattern, which assumes same-level $x_2$-face neighbors. The
\emph{annular policy} makes that assumption true by construction: with FARGO on,
\emph{refined regions must span the full $x_2$ extent} (rings, interior in $x_1$),
enforced fatally via ring-completeness counting. AMR refine/derefine flags are
synchronized ring-wise (OR to refine, AND to derefine) after the global flag exchange.
A runtime guard checks the actual per-block shift against the communicated buffer
depth every step, and the \code{maxjshift} estimate gained the missing $q\Omega$
factor and $2\times$ multilevel headroom. MHD orbital advection (face-centered $B$
via EMF remap) remains fatal-guarded with refinement pending a $\nabla\!\cdot\!B$
analysis. \textbf{Output note:} SMR/AMR runs must write \code{bin} (not \code{vtk})
outputs --- the vtk writer does not support refined meshes; all SMR inputs now
default to \code{bin}.

\subsection{Runs and results}
All runs from \code{validation/run/} with the executable built as in Sec.~4.
Test problems: JG05 incompressible (vortical) shearing wave, \code{ipert=2}
(64$\times$64$\times$4 root grid, $16^3\!\times$4 blocks, level-1 region interior in
$x_1$; the \code{\_ring} input's region spans full $x_2$); epicyclic oscillation,
\code{ipert=1} (spatially uniform velocity field --- exactly preserved by a correct
$y$-shift, so any comm/indexing defect at ring or level boundaries appears as $O(1)$
corruption); and the swing test of Sec.~5.6.

\begin{lstlisting}
ATH=build/src/athena
# 0a: SMR + no-FARGO (reproduces the test/shwave-smr reference run)
$ATH -i inputs/shearing_box/hydro_incompress_shwave_smr.athinput \
     shearing_box/orbital_advection=false
# 0a: uniform grid, FARGO vs non-FARGO consistency
...
$ATH -i inputs/shearing_box/hydro_incompress_shwave.athinput job/basename=shw_nofargo \
     shearing_box/orbital_advection=false
# 0a: swing + FFT self-gravity without FARGO
$ATH -i inputs/tests/swing_selfgrav.athinput job/basename=SwingNoFargo \
     shearing_box/orbital_advection=false
# 0b: FARGO on an annular (full-x2) level-1 ring, and its non-FARGO twin
...
# 0b: epicycle exactness on the ring vs uniform grid
$ATH -i inputs/shearing_box/hydro_incompress_shwave_smr_ring.athinput \
     job/basename=epi_ring problem/ipert=1
$ATH -i inputs/shearing_box/hydro_incompress_shwave.athinput \
     job/basename=epi_unif problem/ipert=1
# guard checks (both must abort with explanatory fatal errors)
$ATH -i inputs/shearing_box/hydro_incompress_shwave_smr.athinput \
     refined_region1/x1min=-0.24 refined_region1/x1max=-0.13   # touches x1 bndry
$ATH -i inputs/shearing_box/hydro_incompress_shwave_smr.athinput \
     shearing_box/orbital_advection=true                       # partial-x2 ring
\end{lstlisting}
...
Swing + gravity, non-FARGO vs linear theory & $L_2(\delta)$ / $\delta_{\sin}$ &
  $1.2\times10^{-2}$ / $10^{-12}$ \\
Non-FARGO cost (swing box, $L_x{=}2\pi$) & cycles vs FARGO & $5.7\times$, as predicted \\
\midrule
Ring + FARGO: mass conservation & max drift & exact (0.0) \\
Epicycle on ring vs analytic $\cos^2(\kappa t)$ & max rel.\ & $5\times10^{-6}$ (RK2) \\
Epicycle on ring vs uniform grid & max rel.\ & 0.12\% ($dt$ difference) \\
Shwave: ring-FARGO vs ring-non-FARGO & $x$-KE & 3.8\% ($\approx$ uniform-grid gap) \\
Boundary-touching refinement & --- & fatal, correct diagnosis \\
Partial-$x_2$ ring with FARGO & --- & fatal, ``4 of 8 MeshBlocks'' \\
\bottomrule
\end{tabular}
\end{center}

The epicycle row is the decisive one: at $5\times10^{-6}$ the only surviving error is
time integration, so the per-level shift and its communications introduce no artifacts.
Not yet exercised: dynamic AMR ring creation/destruction end-to-end (the flag-sync
logic is in place; the natural test arrives with the dust-density refinement criterion),
the FARGO speedup itself in a production-size box, and MHD.
