...
The long-term goal is gas self-gravity in AthenaK shearing-box simulations
\emph{with mesh refinement}. The strategy, in order:
\begin{enumerate}
  \item \textbf{(this work)} A uniform-grid FFT Poisson solver with shearing-periodic
        horizontal boundaries and either periodic or open vertical boundaries.
        FFTs handle the shear-periodic boundary condition exactly (a per-mode phase
        shift), which is why every published shearing-box self-gravity implementation
        (Gammie 2001; Kim, Ostriker \& Stone 2002; Pencil; TIGRESS; NIRVANA) is
        FFT-based.
  \item A swing-amplification physics test (linear theory of self-gravitating
        shearing waves, cf.\ Kim \& Ostriker 2001) exercising the full
        hydrodynamics + gravity loop.
  \item Multi-rank support via \code{MPI\_Allgatherv} at a seam left in the
        gather/scatter steps (Sec.~\ref{sec:impl}).
  \item The refinement hybrid: FFT on the uniform root grid provides the solution
        and the boundary data consistent with shear-periodicity; refined interior
        patches are solved with the existing multigrid module using Dirichlet
        boundary values prolonged from the root solution. Nothing in the present
        solver is discarded in that design.
\end{enumerate}
...
The remap is the conservative flux form shared with AthenaK's orbital advection
(\code{shearing\_box/remap\_fluxes.hpp}): the integer part of the shift is folded
...
\code{inputs/tests/fft\_poisson*.athinput}, \code{jeans\_fft.athinput},
  \code{swing\_selfgrav.athinput} & test inputs \\
