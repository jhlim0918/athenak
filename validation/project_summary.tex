...
$\alpha'$ (eq.~20) diagnostics as history outputs. The standard $\beta = 10$
run \textbf{at their full size and resolution} ($256\times256\times48$, 64
ranks on NAS Athena) reproduces their Figure~2 end to end
(Fig.~\ref{fig:sc14}); over their averaging window ($t = 200$--$300$):
$\alpha_{\rm grav} = 0.0379$ vs.\ their $0.0387$ (\textbf{2\%}), $Q = 1.29$
vs.\ $1.33$, rms $\delta v = 1.71$ vs.\ $1.79\,H\Omega$, $M/M_0 = 0.83$
(consistent), and $\alpha' = 0.038$ within $5\%$ of their eq.-21
energy-balance prediction --- an end-to-end audit of the cooling--heating
budget through the new gravity machinery. A half-width laptop twin isolates a
clean box-width lesson: $32H$ boxes host a single quasi-periodic box-scale
swing mode that inflates the Reynolds channel in bursts and depresses $Q$ ---
worth knowing before shrinking boxes for economy. The $\beta$-scan (their
Table 1, via their ``morphing'' restart chain) is packaged and ready
(\code{scripts/cluster/}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/sc14_b10_hist.png}
\caption{The full-size SC14 $\beta = 10$ run in their Figure-2 form (thick:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/sc14_b10_vol_cut_t300.png}
\caption{$\log_{10}\rho$ at $\Omega t = 300$ in the full-size box, cutaway
volume rendering in the style of SC14's Fig.~2: gravito-turbulent midplane
wakes under the halo they force --- the configuration (stratified, open-$z$,
refinable) that no FFT solver can host.}
\end{figure}
